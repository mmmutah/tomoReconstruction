...
__all__ = ['PanZoomWindow', 'PanAndZoomState', 'rotate', 'rotate_bound', 'find_translation']
...
class PanZoomWindow(object):
    ...
    def __init__(self, img, windowName = 'PanZoomWindow', onLeftClickFunction = None):
        ...
        if img.ndim > 2:
            ...
            cv2.createTrackbar(self.Z_TRACKBAR_NAME, self.WINDOW_NAME, 0, self.TRACKBAR_ZTICKS, self.onZTrackbarMove)
...
#            print "zoomFactor:",zoomFactor
            ...
            if np.any(coordsInDisplayedImage < 0) or np.any(coordsInDisplayedImage > self.panAndZoomState.shape[:2]):
                ...
                if len(self.img.shape) == 2:
                    ...
                    print("this pixel holds ", self.img[int(coordsInFullImage[0]),int(coordsInFullImage[1]), self.Z_selected])
    ...
    def onZTrackbarMove(self, tickPosition):
        ...
        cv2.imshow(self.WINDOW_NAME, frame_selected[int(pzs.ul[0]):int(pzs.ul[0] + pzs.shape[0]),int(pzs.ul[1]):int(pzs.ul[1] + pzs.shape[1])])
...
def find_translation(imgA, imgB, method=3, roiB=None, roiA=None, show=None):
    ...
    if roiA is None:
        (a_top_left_x, a_top_left_y, a_w, a_h) = (0, 0, wa, ha)  # (top_left_x, top_left_y, width, height)
    else:
        (a_top_left_x, a_top_left_y, a_w, a_h) = (roiA[0], roiA[1], roiA[2], roiA[3])

    if roiB is None:
        (b_top_left_x, b_top_left_y, b_w, b_h) = (0, 0, wb, hb)  # (top_left_x, top_left_y, width, height)
    else:
        (b_top_left_x, b_top_left_y, b_w, b_h) = (roiB[0], roiB[1], roiB[2], roiB[3])

    template = imgB[b_top_left_y:(b_top_left_y+b_h), b_top_left_x:(b_top_left_x+b_w)]
    ...
    res = cv2.matchTemplate(imgA[a_top_left_y:a_top_left_y+a_h, a_top_left_x:a_top_left_x+a_w], template, match_algorithm)
    ...
    M = cv2.moments(res[top_left[1] - weight_range:top_left[1] + weight_range, top_left[0] - weight_range:top_left[0] + weight_range])
    if M['m00'] == 0:
        cxf = 0
        cyf = 0
    ...
    if show:
        ...
        # draw roiA
        imgA1 = cv2.rectangle(imgAn, (a_top_left_x, a_top_left_y), (a_top_left_x+a_w, a_top_left_y+a_h), 255, 1)
        imgA1 = cv2.putText(imgA1, 'Searched Area', (a_top_left_x, a_top_left_y), cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 1)
        # draw matched location
        imgA1 = cv2.rectangle(imgA1, (cx, cy), (cx + b_w, cy + b_h), 255, 2)
        imgA1 = cv2.putText(imgA1, 'Matched Area', (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 1)
        # draw roiB
        imgB1 = cv2.rectangle(imgBn, (b_top_left_x, b_top_left_y), (b_top_left_x + b_w, b_top_left_y + b_h), 255, 1)
        imgB1 = cv2.putText(imgB1, 'Template', (b_top_left_x, b_top_left_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, 255, 1)
...
def stitcher(imgstk, shifts, rois, axis=0, slc=None, psroi=[0, 0, 0, 0]):
    ...
    for i in range(0, nframe):     # loop over nframe
        for j in range(0, nvol):  # loop over nvol
            ...
            img1 = cv2.rectangle(img1, (int(shiftx//1 + roi_x - 1), int(shifty//1 + roi_y - 1)), (int(shiftx//1 + roi_x + roi_w + 1), int(shifty//1 + roi_y + roi_h + 1)), 2, 1)
            img1 = cv2.putText(img1, "ROI", (int(shiftx // 1 + roi_x - 1), int(shifty // 1 + roi_y - 1)), cv2.FONT_HERSHEY_COMPLEX, 1, 1)

            # Now copy the transformed image to final array
            arr[i, int(shifty//1 + roi_y):int(shifty//1 + roi_y + roi_h), int(shiftx//1 + roi_x):int(shiftx//1 + roi_x + roi_w)] = \
                img1[int(shifty//1 + roi_y):int(shifty//1 + roi_y + roi_h), int(shiftx//1 + roi_x):int(shiftx//1 + roi_x + roi_w)]
...
def tomo_par_init(dictionary, scid):
    """
    ...
    par = {k: cfg.get(k, None) for k in                           # key without values will be assigned "None"
           ('Notes', 'prefix', 'rot_axis', 'nstitch', 'nproj', 'psroi', 'roi', 'post_recon_roi', 'post_recon_rot', 'focus_check_roi')}

    # default assignment if not available
    par['path_output'] = (cfg['path_output'])#, cfg['recon_dir/'])
    par['rot_angle']   = (cfg['rot_angle'], 0.0)
    par['clim']        = (cfg['clim'], [0, 65535])
    
    # default value if not assigned (TO-DO)

    # create file template
    fname = os.path.join(cfg['path_input'], cfg['prefix'] + '_' + str(1).zfill(cfg['digit']) + '.tif')
    #fname = os.path.join(cfg['path_input'], cfg['prefix'] + '_' +  '.tif')
    par['input_template'] = fname

    #par['roi'] = [cfg['roi_x'][0], cfg['roi_y'][0], cfg['roi_x'][1], cfg['roi_y'][1]]  # [top_left_x, top_left_y, width, height)
    par['theta'] = [float(cfg['theta_start']) , cfg['theta_start'] + cfg['theta_step'] * (cfg['nproj'] -1)]
    par['theta_step'] = cfg['theta_step']
    ...
    for i in range(0,cfg['nstitch']):
        ...
        par['ind_w1'].append(list(range(scno_start,
                                        scno_start + cfg['nwhite1'])))
        par['ind_pj'].append(list(range(scno_start + cfg['nwhite1'],
                                        scno_start + cfg['nwhite1'] + cfg['nproj'])))
        par['ind_w2'].append(list(range(scno_start + cfg['nwhite1'] + cfg['nproj'],
                                        scno_start + cfg['nwhite1'] + cfg['nproj'] + cfg['nwhite2'])))
        par['ind_dk'].append(list(range(scno_start + cfg['nwhite1'] + cfg['nproj'] + cfg['nwhite2'],
                                        scno_start + cfg['nwhite1'] + cfg['nproj'] + cfg['nwhite2'] + cfg['ndark'])))
...
def find_roi(opt):
    ...
    imgw = dxread.read_tiff_stack(fname, ind=ind_to_read, digit=None, slc=None, angle=rot_angle, mblur=medfilter_size)
    imgn = cv2.normalize(imgw[0, :, :], None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    [ch, cw] = imgn.shape

    if roi[2] < cw or roi[3] < ch or roi[0] != 0 or roi[1] != 0:
        cv2.rectangle(imgn, (roi[0], roi[1]), (roi[0] + roi[2], roi[1] + roi[3]), 255, 2, cv2.LINE_AA)
        cv2.putText(imgn, 'current ROI(x,y,w,h): ' + ','.join(map(str, roi)), (roi[0], roi[1] - 5),
